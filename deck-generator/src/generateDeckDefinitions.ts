...
import { quests as QUEST_CATALOG } from './catalog/quests.js';
import {
...
function buildQuestDefinitions(): GeneratedQuestDefinition[] {
	return QUEST_CATALOG.map((quest, index) => {
		return {
			...
			objectives: structuredClone(quest.objectives),
			modifiers:
				quest.modifiers && Object.keys(quest.modifiers).length > 0
					? structuredClone(quest.modifiers)
					: undefined,
		};
	});
}
...
function buildDeckDefinitions(
	...
	const questDefinitions = buildQuestDefinitions();
	const decks: Record<string, GeneratedDeck> = {};
	decks.quests = {
		deck: 'quests',
		cards: questDefinitions.map(toQuestCard),
		consumables: {
			teleport: 0,
			smallHealthPotion: 0,
